...
DATA_PATH = "tasks/jsonl_data/dfd_prompts.jsonl"
...
QUESTIONNAIRE_INSTRUCTION = (
    "In each round, you will be presented with two boxes. "
...
def _make_item_block(row, label_s1: str, label_s2: str) -> str:
    return (
        f"Item {row['id']}:\n"
        f"Box {label_s2}: {str(row['s2']).strip()}\n"
        f"Box {label_s1}: {str(row['s1']).strip()}\n"
...
def get_task_info():
    ...
    return {
        "name": "DFD",
        "description": "DFD gamble-choice task with randomized option labels and canonical s1/s2 mapping",
