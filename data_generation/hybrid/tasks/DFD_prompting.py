#!/usr/bin/env python3
"""
DFD Task Module - Decision from Description Task evaluation using chat template approach.
...
DATA_FILE = "survey_data/prompts_dfd.jsonl"  

def parse_dfd_data(text: str) -> Dict[str, Any]:
    """
    Parse DFD text and extract all information.
    """
    # Extract decision keys from instructions
...
def build_chat_sequence_with_decisions(parsed_data: Dict[str, Any], tokenizer: AutoTokenizer) -> Dict[str, Any]:
    ...
    for problem in problems:
        ...
        problem_start = re.search(rf'Problem {problem_num}:.*?You choose Box', problem_text, re.DOTALL)
        ...
        # Add this problem to the overall sequence
...
def process_DFD_participant_chat_style(participant_data: dict, model: AutoModelForCausalLM, tokenizer: AutoTokenizer, model_key: str, verbose: bool = False) -> List[Dict[str, Any]]:
    """
    Process a single DFD participant using chat-style approach.
    """
    ...
    try:
        # Parse the data
        parsed_data = parse_dfd_data(text)
...
def run_task(model: AutoModelForCausalLM, tokenizer: AutoTokenizer, model_key: str, 
    ...
    """
    Main task runner function for DFD evaluation.
    ...
    """
    logging.info(f"Starting DFD task for model: {model_key}")
    ...
    try:
        ...
        for entry_idx, participant in enumerate(participant_data):
            if entry_idx % 500 == 0:
                logging.info(f"Processing DFD participant {entry_idx + 1}/{len(participant_data)}: {participant.get('participant', 'unknown')}")
            ...
            try:
                ...
                logging.error(f"Error processing DFD participant {entry_idx}: {e}")
                continue
        
        # Create and return DataFrame
        if all_results:
            df = pd.DataFrame(all_results)
            logging.info(f"DFD task completed. Generated {len(all_results)} rows of results.")
            
            return df
        else:
            logging.warning(f"No results generated for DFD task on {model_key}")
            return pd.DataFrame()
            
    except Exception as e:
        logging.error(f"Error in DFD task for model {model_key}: {e}")
        return pd.DataFrame()
